<?php
  ...
  if(!isset($_SESSION['id'])){
	...
	if(isset($_POST["btn1"])){
		...
		$result = $db_handle->runQuery("SELECT ime,prezime,br_pasosa,datum_rodjenja FROM klijent WHERE id_klijent IN (SELECT id_klijenta FROM  rezervacije WHERE id_aranzmana = '$id')");
		$header = $db_handle->runQuery("SELECT `COLUMN_NAME` 
		FROM `INFORMATION_SCHEMA`.`COLUMNS` 
		WHERE `TABLE_SCHEMA`='turistba' 
		AND `TABLE_NAME` = 'klijent' and
		`COLUMN_NAME` in ('ime','prezime','br_pasosa','datum_rodjenja')");
...
<nav class="navbar navbar-inverse" style="max-width:100%; background-color:black;">
  <div class="container-fluid">
      ...
      <ul class="nav navbar-nav" style="font-size:26px;">
		...
		<li class="dropdown">
          ...
          <ul class="dropdown-menu">
			...
			<li class="active"><a href="lista_rezervacija.php">Lista rezervacija</a></li>
	...
	<div class="col-md-offset-3 col-md-6 col-md-offset-3" style="font-size:18px; background-color:#e6f2ff;">
	
			<div style="font-size:18px; text-align:center; background-color: #b3d6ff;"><h2><b>LISTA REZERVACIJA (pdf)</b></h2></div>
		...
		<form method="POST" style="padding-top:10px; padding-bottom:10px; padding-left:5px; padding-right:5px;">		
			<div class="form-body pal">
				...
				<div class="form-group">
					...
					<select class="form-control" name="id" ></li>
						<?php
						
							$sql2="select * from aranzmani where stanje=0";
			...
			<div class="form-actions pal">
				<input class="btn btn-primary" type="submit" name="btn1" value="Lista rezervacija"/>
			</div>
