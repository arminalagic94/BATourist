...
<table class="table" style="font-size:18px; background-color:#e6f2ff;">
    ...
    <?php 
        $sql1="select * from aranzmani where stanje=1";
    ...
    ?>
	    ...
	    <td><?php echo $row['naziv_destinacije'] ?></td>
	    <td><?php echo $row['cijena'] ?> €</td>
	    <td><?php echo $row['akcijska_cijena'] ?> €</td> 
        <td><?php echo $row['br_mjesta'] ?></td>
        <td><?php echo $row['tip'] ?></td>
		<td><?php echo $row['datum_polaska'] ?></td> 
        <td><?php echo $row['tip_prijevoza'] ?></td>
        <td><?php
		
			$sql54="select * from admin where id_admin='$row[id_admina]'";
			$query54=mysql_query($sql54);
			$row2 = mysql_fetch_array($query54); 
			echo $row2["username"];
