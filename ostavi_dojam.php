<?php
  ...
  if(!isset($_SESSION['id'])){
	...
	if(isset($_POST["btn"]))
	{
		...
		$sql2="insert into ocjena_aranzmana(id_ocjena, id_aranzmana, id_klijenta, ocjena, komentar) values(0,'$id_aranzmana','$id_klijenta','$ocjena', '$komentar')";
		$query2=mysql_query($sql2);
...
<li class="dropdown-header"><b>Zimovanje</b></li>
	...
	<div class="col-md-offset-3 col-md-6 col-md-offset-3" style=" background-color:#e6f2ff;">
		...
		<div style="font-size:18px; text-align:center; background-color: #b3d6ff;"><h2><b>OSTAVI DOJAM</b></h2></div>
				...
				<div class="form-group">
					...
					<select class="form-control" name="id_aranzmana" ></li>
						<?php
							...
							$sql3="select * from moje_destinacije where id_klijenta='$id_klijenta' and id_aranzmana in (select id_aranzman from aranzmani where stanje=0) and id_aranzmana not in (select id_aranzmana from ocjena_aranzmana where id_klijenta='$id_klijenta')";
							$query3=mysql_query($sql3);
							while($row1=mysql_fetch_array($query3)){
						
						?>
									<option value="<?php echo $row1["id_aranzmana"]?>"><?php 
									
									$sql5="select * from aranzmani where id_aranzman='$row1[id_aranzmana]'";
									$query5=mysql_query($sql5);
									$row = mysql_fetch_array($query5); 
									echo $row["naziv_destinacije"];?> </option>	<?php
							}
						?>
					</select>
				</div>

				<div class="form-group"><textarea class="form-control" type="text" name="komentar" cols="5" rows="5" placeholder="Unesite komentar aranžmana"></textarea></div>	
...
<table class="table">
  ...
  <div class="carousel-inner" role="listbox">
	...
	<tbody><tr><th><form action="ostavi_dojam.php" method="POST"><button class="btn btn-primary" type="submit" name="btn" value="<?php echo $row['id_aranzman']?>">Rezerviši</button></form></th><td></td></tr>
</tbody>
